<template>
  <div class="container">
    <CmNavBar title="添加用户组成员" />
    <div class="participant">
      <div class="userList">
        <div class="headbox">
          <div>
            <van-checkbox disabled v-model="check1" @click="checkAllAdmin" shape="square"
              >管理员（{{ adminList.length }}人）</van-checkbox
            >
          </div>
        </div>

        <van-checkbox-group disabled v-model="participants1" ref="checkAdmin">
          <van-checkbox
...
<script>
import { categoryUserAdd, groupUserList } from "@/api/organization";
export default {
  name: "addParticipant",
  components: {},
  data() {
  ...
  methods: {
    ...
    goAddMeeting() {
      let obj = {
        ...
        userIds: [...this.participants1, ...this.participants2],
      };
      categoryUserAdd(obj).then((res) => {
        if (res.responseCode === "100000") {
          this.participants1 = [];
          this.participants2 = [];
