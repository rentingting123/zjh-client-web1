pipeline { 
    agent any 
    parameters {
        gitParameter branchFilter: 'origin/(.*)', defaultValue: 'master', selectedValue: 'DEFAULT', name: 'BRANCH_TAG', type: 'PT_BRANCH_TAG', sortMode: 'DESCENDING_SMART', listSize: '5' , description: '请选择要构建的分支或tag'
    }
    environment {
        SKIP_BUNDLING = true
        ...
        //项目的访问地址(域名)
        URL_DOMAIN = 'http://vtrs-client.mycloudedu.net'

        //目标机的名称,需事先在Jenkins系统设置中设置
        PUB_TARGET = 'test_front_01_1.141'

        //构建机上的工作目录
        REMOTE_DIRECTORY = "/data/deploy/test/frontend/vtrs/vtrs_client"

        //部署机上的工作目录
        REMOTE_DIRECTORY_2 = "/data/projects/vtrs/vtrs_client-frontend"
    ...
    stages { 
        stage('Git Checkout') { 
            steps {
                ...
                sh "cp -a ../vtrs_client-frontend-test ${REMOTE_DIRECTORY}/code"
